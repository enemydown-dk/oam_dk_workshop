...
def pull_data_api(url, iden):
    """Execute request in API and print either found data or errorcode to terminal."""
    req = urllib.request.Request(url + iden)
    try:
        response = urllib.request.urlopen(req)
        data = json.loads(response.read())
        write_data(OUTPUT_FILE, data, iden)
        print(data)
        time.sleep(0.5)
...
def open_csv():
    ...
    with open(INPUT_FILE, newline='') as _:
        reader = csv.reader(_, delimiter=',') #Note special delimitor.
        for row in reader:
            ISSN = row[0]
            pull_data_api(URL, ISSN)
            try:
                EISSN = row[1].strip()
                pull_data_api(URL, EISSN)
            except IndexError:
                #not enough elements in the line
                continue
    _.close()

def write_data(json_name, data, iden):
    """Write data to file with name OUTPUT_FILE (append mode)."""
    with open(json_name, mode="a") as file:
        try:
            file.write(iden + ';' + data['results'][0]['bibjson'][SEARCHF]+'\n')
